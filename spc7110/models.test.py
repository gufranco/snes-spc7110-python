...
class CatalogueTest(unittest.TestCase):
    def test_the_catalogue_names_every_mode_the_chip_has(self):
        self.assertEqual(len(models.MODES), 3)

    def test_every_mode_says_what_it_produces(self):
        for mode in models.MODES.values():
            self.assertTrue(mode.summary.strip())

    def test_each_one_names_how_many_bits_a_pixel_takes(self):
        self.assertEqual([mode.depth for mode in models.MODES.values()], [1, 2, 4])

    def test_a_mode_prints_as_something_a_person_can_read(self):
        self.assertIn("1bpp", repr(models.describe("1bpp")))


class NameTest(unittest.TestCase):
    def test_a_mode_is_found_by_its_number(self):
        self.assertEqual(models.describe(0).number, 0)

    def test_and_by_the_depth_it_produces(self):
    ...
    def test_a_name_no_mode_answers_to_is_refused(self):
        with self.assertRaises(models.UnknownMode):
            models.describe("8bpp")

    def test_a_number_the_chip_does_not_have_is_refused(self):
        with self.assertRaises(models.UnknownMode):
            models.describe(3)
