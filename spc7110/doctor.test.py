...
class ExamineTest(unittest.TestCase):
    ...
    def test_and_one_finding_per_mode_it_covers(self):
    ...
    def test_a_mode_that_will_not_build_is_reported_rather_than_hidden(self):
        def boom(_name, _source):
    ...
    def test_a_mode_that_builds_is_reported_with_its_depth(self):
        for one in doctor.examine():
            if one.name == "4bpp":
...
class DeterminismTest(unittest.TestCase):
    ...
    def test_the_report_carries_what_a_known_stream_decodes_to(self):
        self.assertIn("known stream", [one.name for one in doctor.examine()])

    def test_a_decoder_that_will_not_decode_is_reported_rather_than_hidden(self):
        def boom(_name, _source):
    ...
    def test_the_bytes_it_reports_are_the_bytes_it_decoded(self):
        class Counting:
            def __init__(self):
                self.given = 0
...
class EmptyTest(unittest.TestCase):
    def test_a_decompressor_with_nothing_to_decompress_is_refused(self):
        for one in doctor.examine():
            if one.name == "empty stream":
                self.assertTrue(one.ok)

    def test_one_that_accepts_it_anyway_is_a_failure(self):
        found = doctor._empty(lambda _source: object())

        self.assertFalse(found.ok)

    def test_and_one_that_throws_something_else_is_reported_as_what_it_threw(self):
        def boom(_source):
