...
def stream(seed=1, length=4096):
    source = random.Random(seed)
    return bytes(source.randrange(256) for _ in range(length))


def started(mode=0, seed=1, offset=0, index=0):
...
class ModeTest(unittest.TestCase):
    ...
    def test_the_same_stream_decodes_the_same_way_twice(self):
        self.assertEqual(started().read(128), started().read(128))

    def test_a_different_stream_decodes_differently(self):
        self.assertNotEqual(started(seed=1).read(128), started(seed=2).read(128))


class OffsetTest(unittest.TestCase):
    def test_starting_further_along_the_stream_decodes_differently(self):
        self.assertNotEqual(started(offset=0).read(64), started(offset=7).read(64))

    def test_skipping_output_is_the_same_as_reading_and_discarding(self):
    ...
    def test_skipping_nothing_skips_nothing(self):
        self.assertEqual(started(index=0).read(16), started().read(16))


class BufferTest(unittest.TestCase):
    def test_the_buffer_refills_when_it_empties(self):
        found = started()
    ...
    def test_reading_one_byte_at_a_time_gives_the_same_answer(self):
        one = started()
        many = started()

        self.assertEqual(bytes(one.take_byte() for _ in range(64)), many.read(64))

    def test_a_decompressor_that_was_never_started_answers_nothing(self):
        found = decompressor.Decompressor(stream())

        self.assertEqual(found.take_byte(), 0x00)

    def test_the_buffer_size_is_a_power_of_two_the_chip_can_hold(self):
...
class ContextTest(unittest.TestCase):
    def test_a_fresh_start_clears_every_context(self):
        found = started()
        found.read(64)
    ...
    def test_and_clears_which_symbol_each_one_thinks_is_likelier(self):
        found = started()
        found.read(64)
    ...
    def test_decoding_moves_at_least_one_context_off_its_first_row(self):
        found = started()
...
class ReadingTest(unittest.TestCase):
    def test_a_decompressor_prints_as_its_mode_and_where_it_is(self):
        self.assertIn("mode", repr(started()))

    def test_a_context_prints_as_its_row_and_its_flag(self):
