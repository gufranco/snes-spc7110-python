...
DEFAULT_SOURCES = str(Path.home() / ".cache" / "snes-spc7110-reference")
...
def extract(sources, pinned, into):
    ...
    driver has no use for, so the two functions that are the chip are taken and
    the rest is left. The markers come from the pin rather than from here, and a
    file whose text has moved fails loudly instead of yielding something else.
    """
    where = Path(sources) / pinned["source"]
    if not where.exists():
        raise Usage(f"{where} is not there; the reference did not arrive")
    text = where.read_text()
    if pinned["from"] not in text or pinned["until"] not in text:
        raise Usage(f"{where} no longer holds the markers the pin names")
    start = text.index(pinned["from"])
    end = text.index(pinned["until"])
    Path(into).write_text(text[start:end])
    header = Path(sources) / pinned["header"]
    if not header.exists():
        raise Usage(f"{header} is not there; the reference did not arrive")
    (Path(into).parent / pinned["header"]).write_text(header.read_text())
    return Path(into)
...
def run(argv):
    ...
    extract(sources, pinned, ROOT / "ref" / "spc7110_bodies.inc")
