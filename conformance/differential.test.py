...
class CaseTest(unittest.TestCase):
    ...
    def test_a_case_starts_somewhere_in_its_own_stream(self):
        for case in differential.cases(seeds=4):
            self.assertLess(case.offset, len(case.data))

    def test_some_cases_skip_output_before_they_are_read(self):
        self.assertTrue(any(case.index for case in differential.cases(seeds=8)))

    def test_a_case_prints_as_its_mode_and_its_seed(self):
...
class ReplayTest(unittest.TestCase):
    def test_replaying_a_case_answers_as_many_bytes_as_it_asks_for(self):
...
class ComparisonTest(unittest.TestCase):
    ...
    def test_an_answer_that_stops_early_is_reported(self):
        self.assertEqual(differential.disagreement([1, 2], [1])[0], 1)
...
class OptionTest(unittest.TestCase):
    ...
    def test_the_number_of_seeds_can_be_set(self):
        self.assertEqual(differential.options(["--seeds", "5"]).seeds, 5)

    def test_and_the_driver(self):
...
class DriverTest(unittest.TestCase):
    ...
    def test_a_run_of_many_wrong_answers_stops_reporting_after_a_handful(self):
        wrong = self.scripted("#!/bin/sh\ncat > /dev/null\necho 99\n")
...
class AgainstReferenceTest(unittest.TestCase):
    def test_the_model_agrees_with_the_reference_on_every_mode(self):
    ...
    def test_a_short_run_reports_clean(self):
