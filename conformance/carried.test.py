...
that preserved the invariants in `spc7110/tables.py` changed what the model emits
with nothing to notice. No manufacturer document for this part has been found, so
these tables are the whole of what the decoder is steered by.
...
from spc7110 import tables  # noqa: E402
...
class CarriedTableTest(unittest.TestCase):
    ...
    def test_every_table_says_what_it_is_for(self) -> None:
        silent = [entry["name"] for entry in self.entries if not entry.get("what")]
