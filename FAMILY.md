...
Three decompression modes, all three modelled, and the reference they are held to
is pinned by commit and by extract rather than by name, so a check cannot start
comparing against a different implementation without the pin changing first.

Nobody published a document for this part. The top rung of the authority ladder
is empty and the record says so.

It is a part rather than a clocked part, for the same reason the S-DD1 is: a
caller hands it a stream and is handed back what the stream held.

## What is not modelled here

The SPC7110 is more than its decompressor. It also carries a real-time clock and
a memory mapper, and both have homes already:
[snes-rtc-python](https://github.com/gufranco/snes-rtc-python) and
[snes-mapper-python](https://github.com/gufranco/snes-mapper-python). This
repository models the one piece neither of those covers, and the scope is in the
name rather than implied.
