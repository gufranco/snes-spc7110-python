...
**Both upper rungs of the ladder are empty.** No datasheet for the SPC7110 was
published, and no recording of a real one is on this machine. So every constant
in [`conformance/hardware.json`](conformance/hardware.json) carries
`verified: false`, and that is a statement about the sources rather than about
the values.

**The differential is a strong check and it is not a measurement.** It builds
snes9x's decompressor from pinned source, drives both with the same generated
streams, and compares byte for byte. A disagreement is real. An agreement means
two programs agree, and where snes9x is wrong this is wrong with it and nothing
here could tell. That is the first entry in
[`conformance/divergences.json`](conformance/divergences.json).

**One coincidence is written down before anybody cites it.** The output ring
buffer is sixty four bytes, and an eight bit per dot character is also sixty four
bytes. The two are unrelated: this is an internal buffer whose size is tied to no
character format. Left unsaid, the next reader reaches for Nintendo's figure as a
source for it, and a citation for something nobody documented is worse than no
citation at all.

**Nothing here is a timing claim.** How long a decode takes, and whether a console
reading the output can outrun it, is unmodelled and unmeasured. On real hardware
that is what decides whether a game has to wait.
